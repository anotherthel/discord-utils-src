...
class Help(commands.Cog):
    ...
    async def helpa(self, ctx):
        ...
        embed.add_field(name='[]', value='```announce, ping, ping_times, invite, iass, nick_set, add_role, create_role, roles, info, serverinfo, create_tag, tags, del_tag, clear_tags```')
    ...
    async def help2(self, ctx):
        ...
        embed.set_thumbnail(url=url)
        embed.add_field(name='`info`', value='Get info on bot.', inline=True)
        embed.add_field(name='`serverinfo`', value='Get info on current server.', inline=True)
        await ctx.send(embed=embed)


    @commands.command(name='help_page 3', description='Shows help page 3', aliases=['hp_3'])
    ...
    @commands.command(name='help_page 5', description='Shows help page 5.', aliases=['hp_5'])
